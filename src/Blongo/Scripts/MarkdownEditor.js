...
(function($, Markdown) {
    ...
    $(function() {
        ...
        for (var i = 0; i < markdownEditorCount; i++) {
            ...
            var markdownEditor = new Markdown.Editor($input.data("sanitize")
                ? markdownSanitizingConverter
                : markdownConverter,
                i > 0 ? "-" + (i + 1) : null);
            var $markdownStylingLink = $("<a>")
                .html("<small>Styling with Markdown is supported</small>")
                .prop("href", "https://guides.github.com/features/mastering-markdown")
                .prop("target", "_new");
            var $markdownStylingContainer = $("<div>")
                .addClass("m-b-1")
