...
gulp.task('build', function (done) {
    return runSequence('clean', ['fonts', 'icons', 'images', 'scripts', 'styles'], function () {
        done();
    });
});
...
gulp.task('scripts:loadCSS', function () {
    return gulp.src([
            config.paths.src.scripts + '/loadCSS.js',
            config.paths.src.scripts + '/onloadCSS.js'
    ])
...
gulp.task('scripts:respond', function () {
...
// WIP...
gulp.task("globalize-en", function () {
    var locale = 'en';
    ...
    var javaScript = 'Globalize.load(' + jsons.map(function (json) { return JSON.stringify(json); }).join(', ') + ');';

    return fs.writeFile('./globalize.' + locale + '.js', javaScript, function (error) {
        if (error) {
            console.log(error);
        } else {
